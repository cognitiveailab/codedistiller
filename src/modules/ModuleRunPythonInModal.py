...
Keys are accessed in the normal way.  For example `print(data["0"])` will print the item from the `data` working memory with key "0".""",
    ...
    def checkInputRunProgram(self, inputDict:dict):
        ...
        if ('runscript' not in inputDict):
            errors.append("The 'runscript' key is missing from the input data.")
        else:
            # Check that the 'runscript' key is a string
            if (type(inputDict['runscript']) != str):
                errors.append("The 'runscript' key is not a string, but rather a " + str(type(inputDict['runscript'])))
    ...
    def runModalSandbox(self, mount_folder:str, pythonVersion="3.10", apt_packages=["git", "wget", "curl"], workingDir:str="/", runscriptName="echo 'Hello, World!'", requirements_file="requirements.txt", OUTPUT_SUBFOLDER = "results", filesToDownload = ["stdout.python.txt", "stderr.python.txt", "stdout.pip.txt", "stderr.pip.txt", "log.json"], timeout_seconds=600, save_folder="to_save/"):
        ...
        def delete_sandbox_app(appName):
            import subprocess
            try:
                print(f"MODAL DEBUG: Stopping the app (App Name: {appName})")
                res = subprocess.run(
                    ["modal", "app", "stop", appName],   # pass args list to avoid shell issues
                    capture_output=True,
                    text=True,        # strings instead of bytes
                    timeout=60        # optional: avoid hangs
                )
                stdout = res.stdout
                stderr = res.stderr
                exit_code = res.returncode

                # Print the stdout/stderr and exit code
                print(f"MODAL DEBUG: Stopping the app (App Name: {appName})")
                print(f"MODAL DEBUG: stdout:\n{stdout}")
                print(f"MODAL DEBUG: stderr:\n{stderr}")
                print(f"MODAL DEBUG: exit code: {exit_code}")

                if exit_code != 0:
                    error_str = (
                        f"Modal stop failed (rc={exit_code})\n"
                        f"STDOUT:\n{stdout}\n"
                        f"STDERR:\n{stderr}"
                    )
                    print(error_str)
                    ...
                    print(f"Modal stop succeeded (rc=0)\nSTDOUT:\n{stdout}")

            except FileNotFoundError as e:
                error_str = f"Modal CLI not found on PATH: {e}"
                print(error_str)
                sandboxErrors_.append(error_str)
            except Exception as e:
                error_str = "An error occurred while stopping the Modal app: " + str(e) + "\n" + traceback.format_exc()
                print(error_str)
                sandboxErrors_.append(error_str)
    ...
    def actionRunProgram(self, payload:dict):
        ...
        # Step 4A: Output the user-provided runscript
        runscript = inputData.get("runscript", None)
        if (runscript is not None):
            # Write the runscript to a file
            runScriptFileUser = folderOut + "/runscript.sh"
            try:
                with open(runScriptFileUser, "w") as f:
                    f.write(runscript)
                # Change its permission to be executable
                os.chmod(runScriptFileUser, 0o755)
            except Exception as e:
                errors.append("An error occurred while writing the user-provided runscript file: " + str(e) + "\n" + traceback.format_exc())
                return {
                    "input": inputData,
                    "output": None,
                    "errors": errors
        ...
        # Step 4: Add the environment-supplied
        ...
        runScript += "# Now run the user-supplied runscript, and pipe the output to log files.\n"
        runScript += "echo \"Running the user-supplied runscript...\"\n"
        runScript += "# Uses 'source' so it's run in the current shell, not a subshell, and everything in the runscript will persist here.\n"
        runScript += "source ./runscript.sh >stdout.runscript.txt 2>stderr.runscript.txt\n"
        runScript += "echo \"User-supplied runscript completed.\"\n"

        # Create the 'to_save' and 'retain' directories
        runScript += "# Create the 'to_save' and 'retain' directories\n"
        runScript += "mkdir -p to_save\n"
        runScript += "mkdir -p retain\n"

        # Run the main.py file
        runScript += "# Now run the main.py file, and pipe the output to log files.\n"
        runScript += "echo \"Running the main.py file...\"\n"
        runScript += "python main.py >stdout.python.txt 2>stderr.python.txt\n"
        runScript += "echo \"main.py file completed.\"\n"

        runScript += "echo \"Script completed.\"\n"



        # Old runscript (commented out, since the LLM now generates a runscript)
#         runScript = """#!/bin/bash
...
#         """
        ...
        try:
            ...
            result = self.runModalSandbox(mount_folder = folderOut,
                                ...
                                filesToDownload = ["stdout.python.txt", "stderr.python.txt", "stdout.pip.txt", "stderr.pip.txt", "stdout.runscript.txt", "stderr.runscript.txt", "log.json", "results.json"],
...
#            }
        ...
        output = {
            ...
            "runscript.stdout": self.loadLogFile(folderOut + "/" + MODAL_OUTPUT_SUBFOLDER + "/stdout.runscript.txt"),
            "runscript.stderr": self.loadLogFile(folderOut + "/" + MODAL_OUTPUT_SUBFOLDER + "/stderr.runscript.txt"),
            "log": log,
    ...
    def runTestRunPythonInModal(self):
        ...
        payload = {
            "input": {
                ...
                "requirements.txt": "numpy==1.26.4\nscikit-learn==1.3.2\n",
                "code": "import numpy\nprint(\"hello world\")\nwhile(True):\n    pass\n",
                "runscript": "#!/bin/bash\necho \"Running the user-supplied runscript...\"\npip install -r requirements.txt >stdout.pip.txt 2>stderr.pip.txt\n",
            }
        }
